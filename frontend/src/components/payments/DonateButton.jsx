...
const DonateButton = ({
  ...
  const handleDonate = async () => {
    setIsLoading(true);
    try {
      const response = await client.post(ENDPOINTS.PAYMENTS.DONATE, { amount });
      ...
      const msg = error.response?.data?.detail || 'Не удалось создать платёж';
      toast.error(msg);
  ...
  const variantClasses = {
    ...
    ghost: 'bg-transparent hover:bg-surfaceLight text-text-secondary',
  };
